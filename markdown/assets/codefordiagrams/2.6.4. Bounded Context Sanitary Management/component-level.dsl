workspace "AniTec - Sanitary Management Components" "Vistas C4 de componentes del bounded context Sanitary Management." {
    model {
        user = person "Authenticated Mobile User" "Ganadero o veterinario autorizado para usar las capacidades del contexto."
        external1 = softwareSystem "Livestock Context" "Dependencia externa de Sanitary Management."
        external2 = softwareSystem "Veterinary Collaboration Context" "Dependencia externa de Sanitary Management."
        external3 = softwareSystem "Activity Context" "Dependencia externa de Sanitary Management."
        ...
        anitec = softwareSystem "AniTec" "Solución móvil para gestión ganadera." {
            api = container "AniTec REST API" "Backend y fuente autoritativa de reglas de negocio." "C# y ASP.NET Core" {
                apiController = component "REST Interface" "Controladores: HealthEventsController." "ASP.NET Core Controllers"
                apiAssembler = component "Resource Assemblers" "Traduce resources y comandos sin exponer entidades." "C#"
                apiApplication = component "Application Services and Handlers" "Servicios existentes: HealthEventCommandService, HealthEventQueryService. Incluye command handlers y event handlers como diseño objetivo." "C# Application Services"
                apiDomain = component "Sanitary Management Domain Model" "HealthEvent; reglas, value objects y puertos del contexto." "C# Domain Model"
                repositoryPort = component "Repository Ports" "Interfaces de repositorio definidas desde el dominio." "C# Interfaces"
                repositoryAdapter = component "Persistence Adapters" "HealthEventRepository." "Entity Framework Core"
            }
            android = container "Native Android Application" "Cliente Android nativo." "Kotlin y Jetpack Compose" {
                androidUi = component "Sanitary Management Compose UI" "Pantallas y componentes visuales del contexto." "Jetpack Compose"
                androidViewModel = component "ViewModel and UI State" "Mantiene estado de presentación y procesa intenciones." "Kotlin ViewModel"
                androidUseCases = component "Mobile Use Cases" "Casos de uso de registro sanitario, seguimiento clínico y generación de alertas." "Kotlin"
            ...
            flutter = container "Cross-Platform Mobile Application" "Cliente móvil multiplataforma." "Flutter y Dart" {
                flutterUi = component "Sanitary Management Widgets" "Páginas y widgets del contexto." "Flutter"
    ...
    views {
        component api "BC4-ApiComponents" "Componentes backend de Sanitary Management." {
            include *
            autoLayout lr
        }
        component android "BC4-AndroidComponents" "Componentes Android de Sanitary Management." {
            include *
            autoLayout lr
        }
        component flutter "BC4-FlutterComponents" "Componentes Flutter de Sanitary Management." {
            include *
            autoLayout lr
        }
        styles {
